...
//!
//! sponge-based hash popular in blockchain stuff
//!
//! ```rust
...
impl KeccakHasher {
    const MERKLE_PAIR_DOMAIN: &'static [u8] = b"SYLVA_MERKLE_PAIR_KECCAK";
    const ENTRY_DOMAIN: &'static [u8] = b"SYLVA_LEDGER_ENTRY_KECCAK";
    const BYTES_DOMAIN: &'static [u8] = b"SYLVA_BYTES_KECCAK";

    fn keccak_to_hash_output(keccak_hash: [u8; 32]) -> HashOutput {
        HashOutput::new(keccak_hash)
    }

    // add domain prefix to prevent collisions between different hash contexts
    fn create_domain_hasher(domain: &[u8]) -> Keccak256 {
...
impl Hash for KeccakHasher {
    ...
    fn hash_bytes(&self, data: &[u8]) -> Result<HashOutput> {
        ...
        Ok(Self::keccak_to_hash_output(hash_array))
    }
...
mod tests {
    ...
    fn test_context_serialization_deterministic() {
        ...
        let bytes1 = KeccakHasher::serialize_entry_context(&context).unwrap();
        let bytes2 = KeccakHasher::serialize_entry_context(&context).unwrap();
