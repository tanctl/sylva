...
//!
//! industry standard crypto hash with good hardware support
//!
//! ```rust
...
impl Sha256Hasher {
    const MERKLE_PAIR_DOMAIN: &'static [u8] = b"SYLVA_MERKLE_PAIR_SHA2";
    const ENTRY_DOMAIN: &'static [u8] = b"SYLVA_LEDGER_ENTRY_SHA2";
    const BYTES_DOMAIN: &'static [u8] = b"SYLVA_BYTES_SHA2";

    fn sha256_to_hash_output(sha256_hash: [u8; 32]) -> HashOutput {
        HashOutput::new(sha256_hash)
    }

    // add domain prefix to prevent collisions between different hash contexts
    fn create_domain_hasher(domain: &[u8]) -> Sha256 {
...
impl Hash for Sha256Hasher {
    ...
    fn hash_bytes(&self, data: &[u8]) -> Result<HashOutput> {
        ...
        Ok(Self::sha256_to_hash_output(hash_array))
    }
...
mod tests {
    ...
    fn test_context_serialization_deterministic() {
        ...
        let bytes1 = Sha256Hasher::serialize_entry_context(&context).unwrap();
        let bytes2 = Sha256Hasher::serialize_entry_context(&context).unwrap();
